...
def test_write_gives_the_app_directory_and_argv_group_readable_modes_under_any_umask(tmp_path: Path, umask: int) -> None:
    ...
    try:
        TmpfsRunDirPort(tmp_path).write("app", env={"A": "1"}, argv=["x"], cwd="/tmp", project="/tmp")
    finally:
        os.umask(previous)
